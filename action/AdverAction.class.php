<?php
	class AdverAction extends Action {
		...
		if (isset($_POST['send'])) {
			...
			if (Validate::checkLength($_POST['info'],200,'max')) Tool::alertBack('警告：描述长度不得大于200位！');
			...
			if (isset($_GET['id'])) {
				...
				switch ($_adver->type) {
					...
					case 2 :
						...
						$this->_tpl->assign('up',"<input type=\"button\" value=\"上传头部广告690x80\" onclick=\"centerWindow('../confing/upfile.php?type=adver&size=690x80','upfile','400','100')\" />");
						break;
					case 3 :
						$this->_tpl->assign('type3','checked="checked"');
						$this->_tpl->assign('pic','style="display:block"');
						$this->_tpl->assign('up',"<input type=\"button\" value=\"上传侧栏广告270x200\" onclick=\"centerWindow('../confing/upfile.php?type=adver&size=270x200','upfile','400','100')\" />");
						break;		
				}
