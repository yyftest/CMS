  ...
  class UserAction extends Action{
	  ...
	  private function add(){
		  if (isset($_POST['send'])) {
			...
			if(Validate::checkLength($_POST['user'],2,'min')) Tool::alertBack('警告：用户名长度不得小于2位！');
			if(Validate::checkLength($_POST['user'],20,'max')) Tool::alertBack('警告：用户名长度不得大于20位！');
			if(Validate::checkLength($_POST['pass'],6,'min')) Tool::alertBack('警告：密码长度不得小于6位！');
			if(Validate::checkEquals($_POST['pass'],$_POST['notpass'])) Tool::alertBack('警告：密码与确认密码不一致！');
	  ...
	  private function update(){
		  if(isset($_POST['send'])){
			...
			if(!Validate::checkNull($_POST['question']) && !Validate::checkNull($_POST['answer'])){
				...
				$this->_model->updateUser() ? Tool::alertLocation('恭喜您，修改成功！',$_POST['prev_url']):Tool::alertBack('很遗憾，修改失败！');
			}
		if (isset($_GET['id'])) {
	  ...
	  private function state($_state){
		  $_stateArr = array('被封杀的会员','待审核的会员','初级的会员','中级的会员','高级会员','VIP会员');
