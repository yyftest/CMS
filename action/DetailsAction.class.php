  ...
  class DetailsAction extends Action{
	 ...
	 //获取文档详细内容
	  private function getDetails(){
		  if(isset($_GET['id'])){
			  parent::__construct($this->_tpl,new ContentModel());//重写类的实例化
			  $this->_model->id = $_GET['id'];//通过获取模板页面的id
			  if(!$this->_model->getOneContent()) Tool::alertBack('警告：不存在此文档！');
			  $_content = $this->_model->getOneContent();
			  if(!$_content) Tool::alertBack();
  ...
  //获取前台显示的导航
	  private function getNav($_id){
		...
		if($_nav->getOneNav()){
		  //主导航
		  if($_nav->getOneNav()->nnav_name)$_nav2 = '<a href="list.php?id='.$_nav->getOneNav()->iid.'">'.$_nav->getOneNav()->nnav_name.'</a> &gt;';
			$_nav1 = '<a href="list.php?id='.$_nav->getOneNav()->id.'">'.$_nav->getOneNav()->nav_name.'</a>';
			$this->_tpl->assign('nav',$_nav2.$_nav1);
			//子导航列表
