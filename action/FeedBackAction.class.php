  ...
  class FeedBackAction extends Action{
	  ...
	  private function addComment(){
		  if(isset($_POST['send'])){
			  ...
			  if($_url == PREV_URL){
				  ...
				  if(Validate::checkLength($_POST['content'],255,'max')) Tool::alertSBack('警告：评论内容长度不得大于255位！');
				  if (Validate::checkLength($_POST['code'],4,'equals')) Tool::alertBack('警告：验证码码必须是四位！');
				  ...
				  if (Validate::checkLength($_POST['code'],4,'equals')) Tool::alertClose('警告：验证码码必须是四位！');
	...
	private function showComment(){
		if(isset($_GET['cid'])){
			...
			if(!$_content->getOneContent()) Tool::alertBack('警告：不存在此文档的评论！');
			...
			$this->_tpl->assign('getHotTwentyComment',$_object3);
		}else{
	  ...
	  private function setCount(){
		  if(isset($_GET['cid']) && isset($_GET['id']) && isset($_GET['type'])){
			  ...
			  if(!$this->_model->getOneComment()) Tool::alertBack('警告：不存在次评论！');
