  ...
  class ContentAction extends Action{
	  ...
	  private function getPost(){
		  if(Validate::checkNull($_POST['title'])) Tool::alertBack('警告：标题不得为空！');
			  ...
			  if(Validate::checkLength($_POST['tag'],30,'max')) Tool::alertBack('警告：Tag标签长度不得大于30位！');
			  ...
			  if(isset($_POST['attr'])){
			  	$this->_model->attr = implode(',',$_POST['attr']);	//按照","来转换成数组
			  }else{
	  ...
	  private function commend($_commend){
		  ...
		  foreach($_commendArr as $_key=>$_value){
			  ...
			  $_html .= '<input type="radio"'.$_checked.'name="commend" value="'.$_key.'">'.$_value.'</input>';
			  $_checked = "";
		  }
	  ...
	  private function attr($_attr){
		  ...
		  $_attrS = explode(',',$_attr);//被选中的属性。将$_attr字符串按逗号分割成数组
		  $_attrNo = array_diff($_attrArr,$_attrS);//没有被选择的属性
		  if($_attrS[0] != '无属性'){
