...
<form name="content" method="post" action="?action=add">
    ...
    <tr><td>栏　　目：<select name="nav"><option value="" style="padding:0">请选择一个栏目类别</option>{$nav}</select><font color="red">[必选]</font></td></tr>
    ...
    <tr><td>作　　者：<input type="text" name="author" value="{$author}" class="text" />(*作者不得大于10位！)</td></tr>
...
<form name="content" method="post" action="?action=update">
	...
	<table cellspacing="0" class="content">
	<tr><th><strong>发布一条新闻档</strong></th></tr>
    <tr><td>文档标题：<input type="text" name="title" value="{$titlec}" class="text" /><font color="red">[必填]</font>(*标题2-50字符之间)</td></tr>
    <tr><td>栏　　目：<select name="nav"><option value="" style="padding:0">请选择一个栏目类别</option>{$nav}</select><font color="red">[必选]</font></td></tr>
    <tr><td>定义属性：{$attr}
    </td></tr>
    ...
    <tr><td>缩&nbsp; 略 图：<input type="text" name="thumbnail" value="{$thumbnail}" class="text" readonly="readonly" />
                           ...
                           <img name="pic" src="{$thumbnail}" style="display:block;" />(*图片是jpg,gif,png,且200k之内！)
    </td></tr>
    <tr><td>文章来源：<input type="text" name="source" value="{$source}" class="text" />(*文章来源不得大于20位！)</td></tr>
    <tr><td>作　　者：<input type="text" name="author" value="{$author}" class="text" />(*作者不得大于10位！)</td></tr>
    <tr><td><span class="middle">内容摘要：</span><textarea name="info">{$info}</textarea><span>(*内容摘要不得大于200位！)</span></td></tr>
    ...
    <tr><td>文档排序：<select name="sort">
           ...
           　　　　　　　费 金 币：<input type="text" name="gold" value="{$gold}" class="text small" />
    </td></tr>
    <tr><td>阅读权限：<select name="readlimit">
    					{$readlimit}
                   </select>
