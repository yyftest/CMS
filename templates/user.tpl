	...
	<table cellspacing="0" class="user">
		<tr><td>用　户　名：{$user}</td></tr>
           <tr><td>密　　　码：<input type="password" class="text" name="pass" />(*留空则不修改！)</td></tr>
            ...
            <tr><td>电 子 邮 件：<input type="text" class="text" value="{$email}" name="email" /><span class="red">[必填]</span>(*每个电子邮件只能注册一个ID)</td></tr>
            <tr><td>选 择 头 像：<select name="face" onchange="sface();">
            					{$face}
            			</select>
            </td></tr>
            <tr><td><img src="../images/{$facesrc}" name="faceImg" class="face" alt="01.gif" /></td></tr>
            <tr><td>安 全 问 题：
            			<select name="question">
                        	<option value="">没有任何安全问题</option>
                            {$question}
            ...
            <tr><td>问 题 答 案：<input type="text" class="text" value="{$answer}" name="answer" /></td></tr>
            <tr><td>设置权限：
            				{$state}
            </td></tr>
            <tr><td><input type="submit" class="submit" onclick="return checkUpdate();" name="send" onclick="return" value="修改会员" />[<a href="{$prev_url}">返回上一层</a>]</td></tr>
	</table>
</form>
{/if}
