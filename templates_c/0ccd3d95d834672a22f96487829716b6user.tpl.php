	...
	<table cellspacing="0" class="user">
		<tr><td>用　户　名：<input type="text" class="text" name="user" /><span class="red">[必填]</span>(*用户名在2到20位之间)</td></tr>
           <tr><td>密　　　码：<input type="password" class="text" name="pass" /><span class="red">[必填]</span>(*密码不得小于6位)</td></tr>
            <tr><td>密 码 确 认：<input type="password" class="text" name="notpass" /><span class="red">[必填]</span>(*确认密码与密码保持一致）</td></tr>
            <tr><td>电 子 邮 件：<input type="text" class="text" name="email" /><span class="red">[必填]</span>(*每个电子邮件只能注册一个ID)</td></tr>
            <tr><td>选 择 头 像：<select name="face" onchange="sface();">
            ...
            <tr><td><img src="../images/01.gif" name="faceImg" class="face" alt="01.gif" /></td></tr>
            <tr><td>安 全 问 题：
            ...
            <tr><td>问 题 答 案：<input type="text" class="text" name="answer" /></td></tr>
            <tr><td>设置权限：
	...
	<table cellspacing="0" class="user">
		<tr><td>用　户　名：<?php echo $this->_vars['user'];?></td></tr>
           <tr><td>密　　　码：<input type="password" class="text" name="pass" />(*留空则不修改！)</td></tr>
            ...
            <tr><td>电 子 邮 件：<input type="text" class="text" value="<?php echo $this->_vars['email'];?>" name="email" /><span class="red">[必填]</span>(*每个电子邮件只能注册一个ID)</td></tr>
            <tr><td>选 择 头 像：<select name="face" onchange="sface();">
            					<?php echo $this->_vars['face'];?>
            			</select>
            </td></tr>
            <tr><td><img src="../images/<?php echo $this->_vars['facesrc'];?>" name="faceImg" class="face" alt="01.gif" /></td></tr>
            <tr><td>安 全 问 题：
            			<select name="question">
                        	<option value="">没有任何安全问题</option>
            ...
            <tr><td>问 题 答 案：<input type="text" class="text" value="<?php echo $this->_vars['answer'];?>" name="answer" /></td></tr>
            <tr><td>设置权限：
            				<?php echo $this->_vars['state'];?>
            </td></tr>
            <tr><td><input type="submit" class="submit" onclick="return checkUpdate();" name="send" onclick="return" value="修改会员" />[<a href="<?php echo $this->_vars['prev_url'];?>">返回上一层</a>]</td></tr>
	</table>
</form>
<?php } ?>
