...
<table cellspacing="0">
	...
	<tr>
		...
		<td><a href="content.php?action=update&id=<?php echo $value->id ?>">修改</a> | <a href="content.php?action=delete&id=<?php echo $value->id ?>" onclick="return confirm('你真的要删除这个文档吗？')?true:false">删除</a></td>
	</tr>
	<?php }?>
    <?php }else{ ?>
...
<form name="content" method="post" action="?action=add">
	...
	<tr><th><strong>发布一条新闻档</strong></th></tr>
    <tr><td>文档标题：<input type="text" name="title" class="text" /><font color="red">[必填]</font>(*标题2-50字符之间)</td></tr>
    <tr><td>栏　　目：<select name="nav"><option value="" style="padding:0">请选择一个栏目类别</option><?php echo $this->_vars['nav'];?></select><font color="red">[必选]</font></td></tr>
    ...
    <tr><td>缩&nbsp; 略 图：<input type="text" name="thumbnail" class="text" readonly="readonly" />
    					   <input type="button" value="上传缩略图" onclick="centerWindow('../confing/upfile.php?type=content','upfile','400','200')" />
                           <img name="pic" style="display:none;" />(*图片是jpg,gif,png,且200k之内！)
    </td></tr>
    <tr><td>文章来源：<input type="text" name="source" class="text" />(*文章来源不得大于20位！)</td></tr>
    <tr><td>作　　者：<input type="text" name="author" value="<?php echo $this->_vars['author'];?>" class="text" />(*作者不得大于10位！)</td></tr>
    <tr><td><span class="middle">内容摘要：</span><textarea name="info"></textarea><span>(*内容摘要不得大于200位！)</span></td></tr>
    ...
    <tr><td>文档排序：<select name="sort">
           ...
           　　　　　　　费 金 币：<input type="text" name="gold" value="0" class="text small" />
    </td></tr>
...
<form name="content" method="post" action="?action=update">
	...
	<table cellspacing="0" class="content">
	<tr><th><strong>发布一条新闻档</strong></th></tr>
    <tr><td>文档标题：<input type="text" name="title" value="<?php echo $this->_vars['titlec'];?>" class="text" /><font color="red">[必填]</font>(*标题2-50字符之间)</td></tr>
    <tr><td>栏　　目：<select name="nav"><option value="" style="padding:0">请选择一个栏目类别</option><?php echo $this->_vars['nav'];?></select><font color="red">[必选]</font></td></tr>
    ...
    <tr><td>关&nbsp; 键 字：<input type="text" name="keyword" value="<?php echo $this->_vars['keyword'];?>" class="text" />(*每个标签用'，'隔开，总长30位之内！)</td></tr>
    <tr><td>缩&nbsp; 略 图：<input type="text" name="thumbnail" value="<?php echo $this->_vars['thumbnail'];?>" class="text" readonly="readonly" />
                           ...
                           <img name="pic" src="<?php echo $this->_vars['thumbnail'];?>" style="display:block;" />(*图片是jpg,gif,png,且200k之内！)
    </td></tr>
    <tr><td>文章来源：<input type="text" name="source" value="<?php echo $this->_vars['source'];?>" class="text" />(*文章来源不得大于20位！)</td></tr>
    <tr><td>作　　者：<input type="text" name="author" value="<?php echo $this->_vars['author'];?>" class="text" />(*作者不得大于10位！)</td></tr>
    <tr><td><span class="middle">内容摘要：</span><textarea name="info"><?php echo $this->_vars['info'];?></textarea><span>(*内容摘要不得大于200位！)</span></td></tr>
    ...
    <tr><td>评论选项：<?php echo $this->_vars['commend'];?>
             　　　　　　　　浏览次数：<input type="text" name="count" value="<?php echo $this->_vars['count'];?>" class="text small" />
    </td></tr>
    <tr><td>文档排序：<select name="sort">
    					<?php echo $this->_vars['sort'];?>
                   </select>
           　　　　　　　费 金 币：<input type="text" name="gold" value="<?php echo $this->_vars['gold'];?>" class="text small" />
    </td></tr>
    <tr><td>阅读权限：<select name="readlimit">
