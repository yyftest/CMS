...
<script type="text/javascript" src="confing/static.php?type=index"></script>
</head>
...
<?php }else{ ?>
  <?php if(@$this->_vars['login']){ ?>
      ...
      <form method="post" name="login" action="register.php?action=login">
          ...
          <label class="yzm">验证码：<input type="text" name="code" class="text code"/> <img src="confing/code.php" onclick="javascript:this.src='../confing/code.php?tm='+Math.random();" /></label>
...
<div id="news">
    <h3><a href="details.php?id=<?php echo $this->_vars['TopId'];?>" target="_blank"><?php echo $this->_vars['NewTop'];?></a></h3>
    <p>核心提示：<?php echo $this->_vars['TopInfo'];?><a href="details.php?id=<?php echo $this->_vars['TopId'];?>" target="_blank">『查看全文』</a>
    </p>
    <p class="link">
